...
void ParticleAdapter::Update()
{
    ...
    for(const auto& particle : m_Context->GetParticleAdapters())
    {
        ...
        Particle* particlePtr = particleAdapter->GetParticle();


        // glm::lowp_vec2 direction = particlePtr->GetPosition() - m_Position;
        // float distance = glm::length(direction);
        // using basic maths
        resulting_movement.x += particlePtr->GetForce() * (particlePtr->GetPosition().x - m_Position.x);
        resulting_movement.y += particlePtr->GetForce() * (particlePtr->GetPosition().y - m_Position.y);


        // Add the force generated by the particle on the resulting movement
        // Using the force and a distance factor
        
        // resulting_movement += particlePtr->GetForce() * direction;
    ...
    // m_Position += resulting_movement;
    m_Position += glm::normalize(resulting_movement) * 4.0f;
