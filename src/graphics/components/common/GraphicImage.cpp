...
GraphicImage::GraphicImage(GraphicContext* context, const std::string& path, float x, float y, float width, float height) 
    : GraphicObject(context, SHADER_TEXTURE), m_Path(path), m_x(x), m_y(y), m_width(width), m_height(height), 
    m_VAO(0), m_VBO(0), m_EBO(0), m_Texture(0), m_imageloaded(false), m_ToInvert(false)
{
...
void GraphicImage::Update()
{
    ...
    if(!m_imageloaded)
    {
        ...
        if (data)
        {
            if(m_ToInvert)
            {
                // Convert to inverted color with removing alpha channel
...
void GraphicImage::SetWidth(float width)
{
    m_width = width;
    m_IsUpdated = false;
}
void GraphicImage::SetHeight(float height)
{
    m_height = height;
    m_IsUpdated = false;
}

void GraphicImage::SetImage(const std::string& path)
{
    m_Path = path;
    m_imageloaded = false;
    m_IsUpdated = false;
...
void GraphicImage::SetToInvertColor(bool invert)
{
    m_ToInvert = invert;
    m_imageloaded = false;
    m_IsUpdated = false;
